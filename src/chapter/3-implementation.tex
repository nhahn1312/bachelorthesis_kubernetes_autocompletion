...
\subsection{Validierung mit Hilfe des \acs{json}-Schemas}
...
%% Ab hier morgen neu
``JSONValidation'' anhand des Inhalts der \ac{yaml}-Datei wird ein passendes Schema zur Validierung mit der einem Objekt der Klasse ``JSONSchemaService'' gefunden.
Die Auswahl des Schemas erfolgt mit kind und apiVersion aus der \ac{yaml}-Datei. Ist keines oder nur eines der Attribute vorhanden, gibt es kein eindeutiges Ergebnis,
was dem Nutzer durch eine Warnung angezeigt wird.
Ist ein Schema gefunden wird die Validierung durchgeführt und mögliche Fehler werden an den Nutzer gemeldet.
