...
Anschließend wird vom \ac{lsp}-Server die ``doComplete''-Methode des Objekts der Klasse ``YamlLanguageService'' aufgerufen.
...
Die Methode erhält als Übergabeparameter den Inhalt einer Textdatei, die Position des Cursors und das \ac{yaml}-\textit{Document},
in dem sich der Cursor befindet, siehe Zeile 1.
Das Objekt der Klasse ``CompletionsCollectorImpl'' dient zum Speichern der möglichen Autovervollständigungen, siehe Zeile 3.
\\
Bevor diese gefunden werden können, muss der Kontext der Position der Cursors bestimmt werden.
Dieser wird mit der Methode ``getNodeFromPosition'' der Klasse ``YamlDocument'' bestimmt, siehe Zeile 7.
Dafür wird der Knoten im \ac{ast} bestimmt, welcher den Wert enthalten würde, den der Nutzer vervollständigen möchte.
Der bestimmte Knoten könnte \ac{zB} ein \textit{Mapping}-Schlüssel sein.
Zur Autovervollständigung würde der Nutzer Werte für den \textit{Mapping}-Schlüssel vorgeschlagen bekommen.
...
\textbf{Autovervollständigung mit Hilfe des \ac{json}-Schemas}

Das ``SchemaCompletionService'' liefert
